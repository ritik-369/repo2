...
class Engine:
    ...
    def modify_code(self):
        ...
        for line in self.code:
            if self.is_contain(line,"int main()") and ok==1:
                found=1
                ok=0
...
class generator:
    ...
    def small_character(self,start='a',end='z',ends="\n"):
        char=chr(ord('a')+self.number(0,25,prints=0,file=0))
        if self.prints==1:
            print(char,end=ends)
        if self.file==1:
    ...
    def mixed_character(self,start_small='a',end_small='z',start_cap='A',end_cap='Z',ends="\n"):
        char=[self.small_character(start_small,end_small,prints=0,file=0),self.capital_character(start_cap,end_cap,prints=0,file=0)]
    ...
    def vector(self,function,length,start=0,end=1e9,ex_start=-1,ex_end='Z',ends="\n"):
        ...
        for i in range(length):
            if ex_start==-1:
                vectors.append(function(start,end,prints=0,file=0))
            else:
                vectors.append(function(start,end,ex_start,ex_end,prints=0,file=0))
        if self.prints==1:
            for num in vectors:
                print(num,end=" ")
            print(ends)
    ...
    def string(self,function,length,start='a',end='z',ex_start=-1,ex_end='Z',ends="\n"):
        vector=self.vector(function,length,start,end,ex_start,ex_end)
        string=""
        for token in vector:
            token=str(token)
            string+=token
    ...
    def generate_testcases(self,testcase,file=1,prints=1,file_ptr=0):
        ...
        for test in range(testcase):
            ...
            # WRITE FROM HERE
            a=self.number(start=1,end=20,ends=" ")
            b=self.string(function=self.small_character,length=a,start='a',end='z')
            # c=self.vector(self.number,a,-100,100)
            # b=self.vector(function=self.number,length=a,start=0,end=0)
            #ENDS HERE
...
class JUDGE:
    ...
    def process(self):
        ...
        testcases=50
        self.generator=generator(testcases)
